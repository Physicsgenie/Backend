  ...
  public function __construct() {
    ...
    add_action('rest_api_init', function(){
      ...
      */
      ...
      /**
       * @api {post} /activate-problem
       * @apiName ActivateProblem
       * @apiGroup Submit
      ...
      */
      ...
      register_rest_route('physics_genie', 'activate-problem', array(
        'methods' => 'POST',
        'callback' => function($request_data) {
          $json = json_decode($request_data -> get_body());
          ...
          if( $results[0] -> count == 0 ){
            ...
            $wpdb -> update(getTable('pg_user_review'),
              array(
                'active' => 1
              ),
              array(
      ...
      /**
       * @api {post} /deactivate-problem
       * @apiName DeactivateProblem
       * @apiGroup Submit
       * @apiDescription Removes problem from problem pool
       *
       * @apiParam {Number} problem_id ID of problem.
       *
       * @apiSuccess {Number} data 1 on success.
      */
      register_rest_route('physics_genie', 'deactivate-problem', array(
        'methods' => 'POST',
        'callback' => function($request_data) {
          $json = json_decode($request_data -> get_body());
          global $wpdb;
